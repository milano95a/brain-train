...
return totalDamage;},place:function(opts){this.ttWidth=opts.tt.width;this.ttHeight=opts.tt.height;initDebug();debug.clear();this.initScreenBounds();this.initHotZones();if(opts.center===true){return this.placeOverHook(opts);}else{return this._getTooltipPlacement(opts.hk);}},placeOverHook:function(opts){var placement=this._getTooltipPlacement(opts.hk);var centerHook=this._getHookCenterPosition(opts.hk);var distance=centerHook.top.y-centerHook.bottom.y;var tail=opts.ttTailHeight;if(placement.state.indexOf('A')===0){placement.top=(placement.top-distance)+tail;placement.bottom=(placement.bottom-distance)+tail;}else{placement.top=(placement.top+distance)-tail;placement.bottom=(placement.bottom+distance)-tail;}
